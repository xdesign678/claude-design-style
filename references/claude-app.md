...
.model-option-desc { font-size: 12px; color: var(--text-tertiary); }
.model-option.selected { background: var(--bg-muted); }
```
...
.thinking-header.open .icon { transform: rotate(90deg); }
...
.thinking-content.open { grid-template-rows: 1fr; }
.thinking-content > div {
  overflow: hidden;
  padding: 0 16px;
}
.thinking-content.open > div { padding: 0 16px 16px; }
