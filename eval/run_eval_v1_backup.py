...
ANTI_PATTERNS = [
    ...
    ("heavy_shadow", r"rgba\(0,\s*0,\s*0,\s*0\.[3-9]\)|rgba\(0,\s*0,\s*0,\s*[1-9](?:\.\d+)?\)", "Forbidden: shadow opacity > 0.2"),
...
def check_token_accuracy(html: str, test_id: str) -> DimensionScore:
    ...
    if test_id in dark_tests:
        ...
        if has_dark:
            # Extract vars from .dark {} blocks
            dark_blocks = re.findall(r"\.dark\s*\{([^}]+)\}", html, re.DOTALL)
            dark_css = "\n".join(dark_blocks)
            dark_vars = {}
...
def check_typography(html: str) -> DimensionScore:
    ...
    checks += 1
    code_font = re.findall(r"(?:code|pre|\.code)\s*\{[^}]*font-family:\s*([^;]+)", html, re.IGNORECASE | re.DOTALL)
    if code_font:
        if any(s in code_font[0].lower() for s in MONO_FONTS):
            passed += 1
        else:
    ...
    checks += 1
    btn_size = re.findall(r"(?:button|\.btn)[^{]*\{[^}]*font-size:\s*([^;]+)", html, re.IGNORECASE | re.DOTALL)
...
def check_layout(html: str) -> DimensionScore:
    ...
    if max_w:
        ...
        if any(600 <= w <= 660 for w in widths) or any(740 <= w <= 860 for w in widths):
            passed += 1
        else:
